...
%Define $\lSFsymb:  \lwfgS{B}{s}{\act,\l}  \to  \lwfgS{B}{s}{\act,\l}$ as follows.
...
\begin{definition}[$\lVFsymb$] \label{defn:violFixLoc}
%Define $\lVFsymb: \lwfgS{B}{s}{\act,\l}  \to  \lwfgS{B}{s}{\act,\l}$ as follows.
...
Let $\ndv$ be an arbitrary node in $\lSFs{\sD}{\XS}$, so that $\lblocks{\ndv}{\XS}$ holds. There are three cases.\\
...
Hence  $\lblocks{\act}{\YS}$, and so $\act \in \lSFs{\sD}{\YS}$. \\
...
Hence, $\lblocks{\B_i}{\YS}$, and so $\B_i \in \lSFs{\sD}{\YS}$. \\
...
If $\lwfg{B}{s}{\DS}$ is supercycle-free, then by \prop{supercycleGFPLoc}, the set of post-fixpoints of $\lSFsymb$ is empty. 
Hence $\SC = \ewfg$ (this is possible since there may be no border interactions). %CHECK DEF BORDER INTERACTION
If $\lwfg{B}{s}{\DS}$ contains supercycles, then by \prop{supercycleGFPLoc},  the set of post-fixpoints of $\lSFsymb_{\sD}$ is exactly the set of 
...
By \prop{GFPisLargestSC}, \gfp{\SFsymb_{\sD}} is the largest local supercycle in \wfg{\B}{s}. Hence the nodes not in 
