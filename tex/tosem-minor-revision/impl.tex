...
$\lfp{\lVFsymb_{\sD}}$. Its correctness follows immediately from \defn{blocksLoc} and \defn{violFixLoc}.
...
\lion{\RETURNE{\XS}}
\end{tabbing}
\caption{Procedure to compute all supercycle-violations in state $\sD$ of $\dsk{\act}{\l}$}
...
\cmnt\ postcondition: returns  $\lblocks{\sD}{\ndv}{X}$\\
   ...
   \lit{\IFC{\mbox{$v$ is an border interaction $\act$ and } 
                          \neg (\ex \B_i : \act \ar \B_i \in \lwfg{\B}{\sD}{\DS})}}
...
$nodes(\DS)$ be the number of componets and interactions in $\DS$, \ie the number of nodes in $\lwfg{\B}{\sD}{\DS}$ and
...
$O(\card{\DS} + \lwfg{\B}{\tD}{\DS})$, since each edge in $\lwfg{\B}{\tD}{\DS}$ is examined at most once, over all calls of \cLBL{$\ndv, \dsk{\act}{\l}, \sD, \XS$}.
Since $\lwfg{\B}{\tD}{\DS}$ is $O(\card{\DS})$, this is just $O(\card{\DS}$.
%
...
$\Fix$ is $O(nodes(\DS))$, since each iteration either adds a node or reaches the least fixpoint.
Thus the time complexity of  \cLFP{$\DS, \sD$} is $O(nodes(\DS) \mult \card{\DS})$.
%
...
\fig{checkInitSCFree}  presents an algorithm that checks if a given state is supercycle-free.
...
$nodes(\B)$ be the number of components and interactions in $\B$, which is also the number of nodes in $\wfg{\B}{s_0}$, and
...
$O(\card{Q_0} \mult nodes(\B) \mult \card{\B})$.
...
$|\dsks{\act}{\l_\act}|$ be the size of the syntactic description of $\dsks{\act}{\l_\act}$.
%
Then the running  time of \checkLinIntDist{$\B, Q_0, \act, \l$} is $O( |\Mdsk{\act}{\l}| \mult |\dsks{\act}{\l}|)$, since 
$\lwfg{\B}{\sD}{\DS}$ has size in $O(|\dsks{\act}{\l}|)$, and computing in-depth and out-depth in 
$\lwfg{\B}{\sD}{\DS}$ can be done in linear time using depth-first graph search.
%
...
%
The time complexity of \cLconnScV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is $O(nodes(\DS) \mult \card{\DS})$, since maximal strongly conected components
are computable in linear time using Kosaraju's algorithm, and the wait-for paths $\pi$ from $\B_i$ to the border can be checked in linear time by first
removing all nodes with supercycle violations, and then checking if such paths remain. If so, then there is a path from $\B_i$ to the border which
contains no node with a supercycle violation.
%
The time complexity of \cLFV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is also $O(nodes(\DS) \mult \card{\DS})$.

%
The running time of \checkLAOIntDist{$\B, Q_0, \act, \l$} is $O(\card{\Mdsk{\act}{\l}} \mult nodes(\DS) \mult \card{\DS})$,
since \cLFP{$\DS, \sD$} has time complexity in $O(nodes(\DS) \mult \card{\DS})$.
%

The running time of \checkLAOInt{$\B, Q_0, \act$} is $O(\SUM_{1 \le \l \le \l_\act} \card{\Mdsk{\act}{\l}} \mult nodes(\DS) \mult \card{\DS})$
since \checkLAOInt{$\B, Q_0, \act$} iterates \checkLAOIntDist{$\B, Q_0, \act, \l$} until $\l = \l_\act$.
%
The running  time of \checkLAO{$\B, Q_0$} is
$O(\SUM_{\act \in \gamma}  \SUM_{1 \le \l \le \l_\act} \card{\Mdsk{\act}{\l}} \mult nodes(\DS) \mult \card{\DS})$
...
\lio{\FORALLC{\mbox{reachable transitions $\tD \goesto[\act] \sD$ of $\dsk{\act}{\l}$}}}
   ...
   \lit{\FORALLC{\B_i \in \cmps{\act}}}
      \lihc{\IFC{\neg \cLFV{\B_i, \VS, \dsk{\act}{\l},\sD}} \RETURNE{\fff}\ \FI}{\cmnt return $\fff$ if no violation for $\B_i$}
...
\cmnt\ \ie $\B_i$ has a local supercycle formation violation in state $\sD$ of subsystem $\dsk{\act}{\l}$\\
...
\cmnt\ \ie $\B_i$ has a local strong connectedness supercycle formation violation in state $\sD$ of subsystem $\dsk{\act}{\l}$\\
