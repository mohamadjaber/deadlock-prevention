...
\lio{\REPEAT}
   \lit{\mbox{\cmnt loop invariant: at end of $i$'th iteration, $\XS = \lVFi{\ewfg}{i} =  v \in \JOIN_{1 \le d \le i} \lVFsi{\tD}{\ewfg}{d}$}}
...
\cmnt\ postcondition: returns $\lVFs{\tD}{X}$\\ %  in state $t_\DS$ of $\dsk{\act}{\l}$,\\
\\
\lio{\mbox{compute $\lwfg{\B}{\tD}{\dsk{\act}{\l}}$}}
\lio{\YS \gts \ewfg}

\lio{\FORALLC{\ndv \in \dsk{\act}{\l}}}

   \lit{\IFC{\mbox{$v$ is an interior interaction $\actp$ and } 
                          \neg (\ex \B_i : \actp \ar \B_i \in \lwfg{\B}{\tD}{\dsk{\act}{\l}})}}
    ...
    \lit{\ELSFC{\mbox{$v$ is an interior interaction $\actp$ and } 
     (\fa \B_i : \actp \ar \B_i \in \lwfg{\B}{\tD}{\dsk{\act}{\l}}  :  B_i \in \XS )}}
        \lihc{\YS \gts \YS \join \set{\ndv}}{\cmnt all outgoing wait-for-edges go to nodes in $\XS$}

    \lit{\ELSFC{\mbox{$v$ is a component $\B_i$ and }
     (\ex \actp : \B_i \ar \actp \in \lwfg{\B}{\tD}{\dsk{\act}{\l}}  : \actp \in \XS )}}
...
The time complexity of \cLFP{$\DS, \tD$} is then $O(\Fix \mult (\card{\lwfg{\B}{\tD}{\DS}} + \card{\DS} \mult \Adj))$
