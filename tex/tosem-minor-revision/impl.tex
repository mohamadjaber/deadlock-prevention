...
The time complexity of \cVL{$\DS, \sD, \XS$} is 
$O(\card{\lwfg{\B}{\sD}{\DS}} + \card{\DS} \mult \Adj)$
where
...
where $\Fix$ is the number of iterations that  \cLFP{$\DS, \sD$} takes to reach a fixpoint. In the worst case, 
$\Fix$ is $O(\card{\lwfg{\B}{\sD}{\DS}})$.
...
The time complexity of \cInitSCFree{$Q_0$} is
%$O(\card{Q_0} \mult \Fix \mult (\card{\wfg{\B}{s_0}} + \card{\B} \mult \Adj)$
$O(\SUM_{s_0 \in Q_0} \Fix(s_0) \mult (\card{\wfg{\B}{s_0}} + \card{\B} \mult \Adj)$
where
%$\card{Q_0}$ is the number of states in $Q_0$, \ie the number of initial states,
...
$\Fix(s_0)$ is the  number of iterations that  \cLFP{$\B, s_0$} takes to reach a fixpoint. In the worst case, 
