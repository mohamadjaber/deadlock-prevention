...
$\cnodes{\DS}$ be the number of componets and interactions in $\DS$, \ie the number of nodes in $\lwfg{\B}{\sD}{\DS}$ and
...
%
The time complexity of \cLconnScV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is $O(\cnodes{\DS} \mult \card{\DS})$, since maximal strongly conected components
are computable in linear time using Kosaraju's algorithm, and the existence of paths in $\WL$ from $\B_i$ to the border of $\DS$ can be checked in linear time by 
using depth-first graph search. 
%
The time complexity of \cLFV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is also $O(\cnodes{\DS} \mult \card{\DS})$.
%
The running time of \checkLAOIntDist{$\B, Q_0, \act, \l$} is $O(\card{\Mdsk{\act}{\l}} \mult \cnodes{\DS} \mult \card{\DS})$,
since \cLFP{$\DS, \sD$} has time complexity in $O(nodes(\DS) \mult \card{\DS})$.
%
The running time of \checkLAOInt{$\B, Q_0, \act$} is $O(\SUM_{1 \le \l \le \l_\act} \card{\Mdsk{\act}{\l}} \mult \cnodes{\DS} \mult \card{\DS})$
since \checkLAOInt{$\B, Q_0, \act$} iterates \checkLAOIntDist{$\B, Q_0, \act, \l$} until $\l = \l_\act$.
%
The running  time of \checkLAO{$\B, Q_0$} is
$O(\SUM_{\act \in \gamma}  \SUM_{1 \le \l \le \l_\act} \card{\Mdsk{\act}{\l}} \mult \cnodes{\DS} \mult \card{\DS})$
