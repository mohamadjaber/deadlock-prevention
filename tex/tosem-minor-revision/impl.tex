...
$O(\SUM_{\act \in \gamma}    \SUM_{1 \le \l \le \l_\act}   \    |\Mdsk{\act}{\l}| * |\dsks{\act}{\l}|)$.
...
$|\dsks{\act}{\l_\act}|$ be the size of the syntactic description of $\dsks{\act}{\l_\act}$.
%
The running time of \cLconnScV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is $O(nodes(\DS) \mult \card{\DS})$, since maximal strongly conected components
are computable inlieanr time using Kosaraju;'s algorithm, and the wait-fir paths from $\B_i$ to the border can be checked in lear time bny first
removing all nodes with supercycle violations, and then checking if such paths remain.....
...
The running time of \cLFV{$\B_i, \VS, \dsk{\act}{\l}, \sD$} is 
The running time of \checkLAOIntDist{$\B, Q_0, \act, \l$} is
The running time of \checkLAOInt{$\B, Q_0, \act$} is 
The running time of \checkLAO{$\B, Q_0$} is 
...
\lio{\FORALLC{\mbox{reachable transitions $\tD \goesto[\act] \sD$ of $\dsk{\act}{\l}$}}}
   ...
   \lit{\FORALLC{\B_i \in \cmps{\act}}}
      \lihc{\IF\ \neg \cLFV{\B_i, \VS, \dsk{\act}{\l},\sD}\ \THEN\ \RETURNE{\fff}\ \FI}{\cmnt return $\fff$ if no violation for $\B_i$}
...
\lio{\mbox{remove from $\lwfg{B}{\sD}{\DS}$ all nodes in $\VS$, \ie with a local supercycle violation}}
