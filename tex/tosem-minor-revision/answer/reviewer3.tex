...
~
    ...
    \answer{We thank the reviewer for the very valuable insight. The paper has been significantly rewritten to use fixpoints and that helped simplify the presentation as expected by the reviewer.}
\end{enumerate}
...
\answer{
In the paper, we introduce behavioral restriction condition per interaction (Definition 3.11). Then,   in Theorem 3.13, we introduce deadlock-freedom via supercycle-freedom preserving restriction by checking behavioral restriction on every interaction. Therefore, if the system is symmetry, it is sufficient to check one representative of each symmetry class. 
}
