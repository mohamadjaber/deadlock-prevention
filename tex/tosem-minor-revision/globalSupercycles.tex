...
Let $\ndv$ be an arbitrary node in $\SFs{s}{\XS}$, so that $\blocks{\ndv}{\XS}$ holds. There are two cases.\\
...
Hence  $\blocks{\act}{\YS}$, and so $\act \in \SFs{s}{\YS}$. \\
...
Hence $\blocks{\B_i}{\YS}$, and so $\B_i \in \SFs{s}{\YS}$. \\
...
Hence $\neg \blocks{v}{\compl{ \VFsi{s}{\ewfg}{d-1} } }$.
