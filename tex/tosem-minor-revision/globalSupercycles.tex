...
$\VFs$ is monotone and continuous, its least fixpoint can be computed as usual by iterating $\VFs$, starting from \ewfg.
...
of $\VFs$. Also let $\JOIN$ be the ``quantifier'' version of $\join$.
Note that $\VFi{\ewfg}{d} \subg \VFi{\ewfg}{d'}$ when $d \le d'$, since $\VFs$
is monotone.
...
$\scV{v}{s} \df v \in \lfp{\VFs}$ and, for $d \ge 1$,
...
$\scVd{v}{d}{s}$ defines a supercycle violation that can be confirmed within $d$ iterations of $\VFs$, which we call a \emph{level-$d$ supercycle violation}.
$\scV{v}{s}$ requires, in general, the entire least fixed point of $\VFs$.


\vspace{1ex}
\begin{example}[Supercycle violation]
For example, consider \fig{SCnotCycle}. We show the set of nodes in each $\VFi{\ewfg}{d}$, as the induced subgraph is easily inferred from \fig{SCnotCycle}.
$\VFs^{0} (\ewfg) = \ewfg$,
$\VFs^{1} (\ewfg) = \set{\acti}$, 
$\VFs^{2} (\ewfg) = \set{\B_6, \acti}$, 
$\VFs^{3} (\ewfg) = \set{\acth, \B_6, \acti}$, 
$\VFs^{4} (\ewfg) = \set{\B_5, \acth, \B_6, \acti}$, 
$\VFs^{5} (\ewfg) = \set{\B_5, \acth, \B_6, \acti}$, 
as so $\lfp{\VFs} = \set{\B_5, \acth, \B_6, \acti}$.
%
...
\begin{example}[Supercycle violation]
Figure~\ref{fig:globalViolations} illustrates supercycle violations in four global states of the dining philosophers system of \fig{diningSpectrum}.
...
For each node $\ndv$ (interaction or component), we include a small positive integer after its name, giving the smallest $d$ such that $v \in \VFi{\ewfg}{d}$.
\end{example}


\begin{figure*}[ht]
  \begin{center}
      \subfigure[Supercycle violations in initial state.]{\label{fig:violsInitial}\scalebox{0.35}{\input{figs/scvDiningInitial.pdf_t}}} \quad \quad
      \subfigure[Supercycle violations after execution of $\Grab_0$.]{\label{fig:violsGrab}\scalebox{0.35}{\input{figs/scvDining1.pdf_t}}} 
      \subfigure[Supercycle violations after execution of $\Grab_0; \Grab_2$.]{\label{fig:violsGrabGrab}\scalebox{0.38}{\input{figs/scvDining2.pdf_t}}} \quad \quad
      \subfigure[Supercycle violations after execution of $\Grab_0; \Grab_2; \Rel_0$.]{\label{fig:violsGrabGrabRel}\scalebox{0.38}{\input{figs/scvDining3.pdf_t}}} 
      \caption{Example supercycle violations for dining philosophers system of Figure~\ref{fig:diningSpectrum}.}
       \label{fig:globalViolations}
  \end{center}
\end{figure*}
