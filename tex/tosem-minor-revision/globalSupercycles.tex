...
\begin{definition}[$\VFsymb$]  \label{defn:violFix}
Define $\VFsymb: \wfgS{\B}{s} \to \wfgS{\B}{s}$ as follows.
$\VF{\XS}$ is the subgraph with nodes
$\set{v \stt \neg \blocks{v}{\compl{\XS}}}$, together with their induced edges.
\end{definition}
%
Hence $\VF{\XS} = \overline{\SF{\overline{\XS}}}$, \ie $\VFsymb$ and $\SFsymb$ are duals.
Note that $\SFsymb$ and $\VFsymb$ are defined given a particular BIP system 
$\B$ and a particular state $s$ of $\B$. Hence we should really write 
$\SFsymb_{\B,s}(\XS)$, $\VFsymb_{\B,s}(\XS)$ to indicate this functional dependence. We
...
$\SFsymb$ and $\VFsymb$ are monotone and continuous.
\end{proposition}
%
\begin{proof}
% show its monotnoe
We show first that $\SFsymb$ is monotone, \ie $\XS \subg \YS \imp \SF{\XS} \subg \SF{\YS}$.
...
$\SFsymb$ is monotone.
%
Since the dual of a monotone mapping in a complete boolean lattice is also monotone, we have that $\VFsymb$ is monotone.
%
Finally, since $\lat{\B}{s}$ is finite, it follows that $\SFsymb$ and $\VFsymb$ are continuous.
\end{proof}

Hence, by the Knaster-Tarski theorem, the least and greatest fixpoints of $\SFsymb$ and $\VFsymb$ exist.
...
Let $\SC$ be the greatest fixpoint of $\SFsymb$. Then either
...
If \wfg{\B}{s} is supercycle-free, then by \prop{supercycleGFP}, the set of post-fixpoints of $\SFsymb$ is empty. 
Hence $\SC = \ewfg$.
If \wfg{\B}{s} contains supercycles, then by \prop{supercycleGFP},  the set of post-fixpoints of $\SFsymb$ is exactly the set of 
...
$\ndv \in  \lfp{\VFsymb}$ iff $\ndv$ is not a node in any supercycle of \wfg{\B}{s}.    
%Let  $\VS = \lfp{\VFsymb}$, \ie $\VS$ is the least fixpoint of $\VFsymb$. Then $v \in \VS$ iff $v$ is not a node in any supercycle of \wfg{\B}{s}.    
...
\lfp{\VFsymb} = \compl{\gfp{\SFsymb}}.
By \prop{GFPisLargestSC}, \gfp{\SFsymb} is the largest supercycle in \wfg{\B}{s}. Hence the nodes not in 
\gfp{\SFsymb} are exactly the nodes that are not in any supercycle. These are exactly the nodes in \lfp{\VFsymb}.
\end{proof}
...
Note that $\VFi{\ewfg}{d} \subg \VFi{\ewfg}{d'}$ when $d \le d'$, since $\VFsymb$
is monotone.
Hence $\VFi{\ewfg}{1}, \VFi{\ewfg}{2}, \ldots$ is a non-decreasing sequence.

\begin{proposition}  \label{prop:computeLFP}
$\lfp{\VFsymb} = \JOIN_{d \ge 1} \VFi{\ewfg}{d}$.
\end{proposition}
%
\begin{proof}
By \prop{monotone}, $\VFsymb$ is continuous. Follows by standard results, \eg see the CPO fixpoint theorem I in 
\citeN{DP02}.
\end{proof}
...
$\scV{v}{s} \df v \in \lfp{\VFsymb}$ and, for $d \ge 1$,
$\scVd{v}{d}{s} \df v \in \VFsymb^{d} (\ewfg)$.\footnote{Note that
...
By \prop{computeLFP}, $\ndv \in \lfp{\VFsymb} \ev v \in \JOIN_{d \ge 1} \VFi{\ewfg}{d}$.
By \defn{supercycle.violation}, $\fa d \ge 1: (\scVd{v}{d}{s} \ev v \in \VFsymb^{d} (\ewfg))$.
...
By \prop{computeLFP}, we can compute $\lfp{\VFsymb}$ (and therefore $\scV{v}{s}$) by iterating $\VFsymb$, starting from $\ewfg$, until there is no more change.
$\scVd{v}{d}{s}$ defines a supercycle violation that can be confirmed within $d$ iterations of $\VFsymb$, which we call a \emph{level-$d$ supercycle violation}.
$\scV{v}{s}$ requires, in general, the entire least fixed point of $\VFsymb$.


\vspace{1ex}
\begin{example}[Supercycle violation]
For example, consider \fig{SCnotCycle}. We show the set of nodes in each $\VFi{\ewfg}{d}$, since the induced subgraph is easily inferred from \fig{SCnotCycle}.
%$\VFsymb^{0} (\ewfg) = \ewfg$,
$\VFsymb^{1} (\ewfg) = \set{\acti}$, 
$\VFsymb^{2} (\ewfg) = \set{\B_6, \acti}$, 
$\VFsymb^{3} (\ewfg) = \set{\acth, \B_6, \acti}$, 
$\VFsymb^{4} (\ewfg) = \set{\B_5, \acth, \B_6, \acti}$, 
$\VFsymb^{5} (\ewfg) = \set{\B_5, \acth, \B_6, \acti}$, 
...
$v$ has no outgoing wait-for-edges in $\wfg{\B}{s}$. Hence $\neg \blocks{v}{\wfg{\B}{s}}$, \ie $v$ is not blocked by the entire set of nodes in 
$\wfg{\B}{s}$. Hence $\neg \blocks{v}{\compl{\ewfg}}$, since $\wfg{\B}{s} = \compl{\ewfg}$. So by \defn{violFix}, 
...
By the induction hypothesis applied to $d'$, we obtain $\viol{w}{d'}{s}$, and so $w \in \VFsymb^{d'} (\ewfg)$ by \defn{supercycle.violation}.
Hence $w \in \VFsymb^{d-1} (\ewfg)$, since, by monotonicity of $\VFsymb$, we have $\VFsymb^{d'}(\ewfg) \subg \VFsymb^{d}(\ewfg)$ when $d' \le d$.
Since  $w$ is an arbitrary successor of $v$, it follows that $v$ is only blocked by nodes in $\VFsymb^{d-1} (\ewfg)$.
Hence $\neg \blocks{v}{\compl{\VFsymb^{d-1} (\ewfg)}}$. 
By \defn{violFix}, $v \in \VF{\VFsymb^{d-1} (\ewfg)}$, \ie $v \in \VFsymb^{d} (\ewfg)$.
...
%
% Alternatively, observe that, by an easy induction, $v$ has out-depth
% $d$ implies that $v \in \JOIN_{d \ge 0} \VFsymb^{d} (\ewfg)$.
