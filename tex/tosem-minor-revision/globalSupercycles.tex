...
%
For \fig{cycleOK}, it is easy to verify that $\lfp{\VFs}$ consists of all the nodes in the system, \ie the wait-for graph shown is supercycle-free.
\end{example}
...
By the induction hypothesis applied to $d'$, we obtain $\viol{w}{d'}{s}$, and so $w \in \VFs^{d'} (\ewfg)$ by \defn{supercycle.violation}.
