...
\cmnt\ compute least fixpoint of $\lVFs$ in state $t_\act$ of $\dsk{\act}{\l}$, \ie $\lfp{\lVFs} = \UN_{d \ge 0} \lVFs^{d} (\ewfg)$.\\
%\cmnt\ Precondition: $\fas v \in \dsk{\act}{\l}: V[v] = \fff$ \\
\cmnt\ postcondition: returns array $V$ such that $\fas v \in \dsk{\act}{\l}: V[v]$ 
$ =
  \begin{cases}
    \ttt     & \quad \text{if }  v \in \lfp{\lVFs}\\
...
\lio{\fpt \gts \fff} 
\lio{\WHILE~\neg \fpt}
   \lit{\mbox{\cmnt loop invariant: on the $i$'th iteration, $V[v] \equiv  v \in \UN_{0 \le d \le i} \lVFs^{d} (\ewfg)$}}
   \lit{\fpt \gts \ttt} 
   \lit{\FORALLC{v \in \dsk{\act}{\l} \land \neg V[v] }}
     ...
     (\fa \B_i : \actp \ar \B_i \in \wfg{\dsk{\act}{\l}}{t_\act} :  V[B_i] )}}
                  \lifc{V[v] \gts \ttt}{\cmnt all outgoing wait-for-edges go to violating nodes}
                  \lif{\fpt \gts \fff} 
     \lih{\ELSFC{\mbox{$v$ is a component $\B_i$ and }
     (\ex \actp : \B_i \ar \actp \in \wfg{\dsk{\act}{\l}}{t_\act} : V[\actp] )}}
                  \lifc{V[v] \gts \ttt}{\cmnt some outgoing wait-for-edge goes to a violating node}
                  \lif{\fpt \gts \fff} 
    \lih{\FI}
  \lit{\ENDFOR}
\lio{\ENDWHILE} 
