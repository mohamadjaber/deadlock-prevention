...
components $B_1, B_2,B_3$, and their interactions $a,b,c,d$, is in bold.  The supercycle can be extended to contain $B_4$, but neither $B_5$ nor
$B_6$: $B_6$ is enabled, and $B_5$ is ready to execute the interaction $h$, which waits only for $B_6$.
%
\begin{figure}[ht]
\begin{center}
...
corresponding to the $\Grab_0$ interaction with a distance of $1$, i.e., its components and their interactions. 
The subsystem includes the $\Grab_0$ interaction, 
components $\Phl_0, F_0,$ and $F_1$ involved in $\Grab_0$, 
and interactions $\Rel_0, \Grab_1,\Rel_1, \Grab_3,$ and $\Rel_3$ connected to these components. 
...
the border interactions (underlined in the figure)   as blocking interactions. 
