...
In practice, we describe the transition system using some syntax, 
\eg involving variables.  
We abstract away from issues of syntactic description since we are only 
interested in enablement of ports and actions. 
%
We assume that enablement of a
port depends only on the local state of a component. 
In particular, it cannot depend on the state of other
components. 
\redbox{
For example, state $e$ in atomic component $P$ of Figure~\ref{fig:components} 
enables port \release~ as there exists a transition from $e$ labeled with
port \release.} 
%This assumption is a restriction on BIP, and we defer to subsequent work 
%how to lift this restriction.  
So, we assume the existence of a predicate $\gd{p_i}{i}$ that holds in 
state $s_i$ of component $\B_i$ iff port $p_i$ is enabled in
...
For example, interaction $a_{001}$ connects ports \get, $\use_l$ and $\use_r$
from components $P_0$, $F_0$ and $F_1$ and corresponds to 
philosopher component $P_0$ acquiring both forks and moving to the eating state. }
%
...
\redbox{
For example, interaction $a_{001}=\{\get_0, \use_{l0}, \use_{r1}\}$ and 
...
Let the initial state be $(h,h,h,h,f,f,f,f)$, i.e., all philosophers are hungry and all forks are free. Also, $\{P_0.get, F_0.use_l, F_1.use_r\}$ (resp.  $\{P_2.get, F_2.use_l, F_3.use_r\}$) is the interaction in which $P_0$ (resp. $P_2$) picks up both forks, and $\{P_0.release, F_0.free_l, F_1.free_r\}$ is the interaction in which $P_2$ releases both forks. 
Consider the following execution: $\rho= (h,h,h,h,f,f,f,f)$  $\{P_0.get, F_0.use_l, F_1.use_r\}$  $(e,h,h,h,u_l,u_r,f,f)$  $\{P_2.get, F_2.use_l, F_3.use_r\}$ $(e,h,e,h,u_l,u_r,u_l,u_r)$ $\{P_0.release, F_0.free_l, F_1.free_r\}$  $(h,h,e,h,f,f,u_l,u_r)$  \ldots
%, i.e., philosopher $P_0$ and $P_2$ gets their left and right forks, respectively, then $P_0$ releases its left and right forks. 
The projection of this execution on the sybsystem defined by subcomponent $\{P_0, P_1, F_0\}$, is equal to:
$\rho \pj (\{P_0, P_1, F_0\}, Q'_0) = (h,h,f) \, \{P_0.get, F_0.use_l\} \, (e,h,u_l) \, \{P_0.release, F_0.free_l\} \, (h,h,f) \ldots$. In particular, we project the states and interactions with respect to the subcomponent. Notice that, interaction $Get_2$ will disappear as its ports do not belong to the subcomponent.
