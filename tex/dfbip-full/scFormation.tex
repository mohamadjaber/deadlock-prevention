...
aa\= mm\= mm\= mm\= mm\=\kill
$\viol{v}{d}{t}$\\
\lioc{\IFC{d = 1 \land \neg \ex u : v \ar u \in \wfg{\B}{t}}\ \RETURNE{\ttt}\ \FI}{\cmnt base case for true}

\lio{\IFC{\mbox{$v = \B_i$} \land (\ex \act : \B_i \ar \act \in \wfg{\B}{t} : (\ex d' < d : \viol{\act}{d'}{t}))} \ \RETURNE{\ttt}\ \FI}

\lio{\IFC{\mbox{$v = \act$} \land (\fa \B_i : \act \ar \B_i \in \wfg{\B}{t} : (\ex d' < d : \viol{\B_i}{d'}{t}))} \  \RETURNE{\ttt}\ \FI}

\lioc{\RETURNE{\fff}}{\cmnt no case for true value, so value is false}
