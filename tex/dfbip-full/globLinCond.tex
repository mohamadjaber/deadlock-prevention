...
Consider a reachable transition
...
\noindent
\ul{Inductive step, $d > 1$.}
By Definition~\ref{def:path} and \ref{def:depth}, all successors (\ie nodes $u$ such that $v \ar u \in \wfg{\B}{t}$)
of $v$ have an in-depth of at most $d-1$. Hence, by the induction hypothesis applied to $d-1$, we obtain 
$\viol{u}{d-1}{t}$ for all predecessors $u$ of $v$. Hence by Definition~\ref{def:supercycle.violation}, 
Clauses~\ref{def:supercycle.violation.component.out} and \ref{def:supercycle.violation.interaction.out},
$\viol{v}{1}{t}$.
}
...
\ind For every reachable transition $s \goesto[\act] t$ of $(\B, Q_0)$,\\
\ind \ind  $\fas \B_i \in \cmps{\act}, \ex d \ge 1: \viol{\B_i}{d}{t} \lor \connViol{v}{t}$.

\noindent
Hence by Definition~\ref{def:formation.violation}, \\
\ind For every reachable transition $s \goesto[\act] t$ of $(\B, Q_0)$,\\
\ind \ind  $\fas \B_i \in \cmps{\act}, \ex d \ge 1: \formViol{v}{t}$
